...
def compare_classifiers(data, target, k=5):
    # Объединение данных
    data_A = pd.DataFrame({'x2': A_x1, 'x3': A_x3})
    data_B = pd.DataFrame({'x2': B_x1, 'x3': B_x3})
    ...
    for i, (true_label, lda_predicted_label, knn_predicted_label) in enumerate(zip(y_test, lda_pred, knn_pred)):
        if true_label != lda_predicted_label:
            lda_errors.append((i, X_test.iloc[i]['x2'], X_test.iloc[i]['x3']))
        if true_label != knn_predicted_label:
            knn_errors.append((i, X_test.iloc[i]['x2'], X_test.iloc[i]['x3']))

    print("LDA errors:")
    for error in lda_errors:
        print(f"Index: {error[0]}, x2: {error[1]}, x3: {error[2]}")
...
A_x1 = excel_data['A_x2'].tolist()
A_x3 = excel_data['A_x3'].tolist()
B_x1 = excel_data['B_x2'].tolist()
